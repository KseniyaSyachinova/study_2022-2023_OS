...
Для начала изучим кмонады для архивации. Для этого используем команду "man". (рис. @fig:001),(рис. @fig:002),(рис. @fig:003),(рис. @fig:004).

![Команды](image/1.png){#fig:001 width=50%}
...
Далее создадим файл, в которм будет написан скрипт. Откроем его с помощью редактора "emacs" (сочетание клавиш "c+x", "c+f").(рис. @fig:005),(рис. @fig:006).

![Создание файла](image/5.png){#fig:005 width=50%}

![Открытие файла](image/6.png){#fig:006 width=50%}

Напишем скрипт согласно задания. При написании скрипта я буду использовать архиватор "bzip2".(рис. @fig:007).

![Текст скрипта](image/7.png){#fig:007 width=50%}

Сохраняем файл. Добавим право на выполнение с помощью команды "chmod +x *.sh" и проверим работу скрипта. Также проверим, появился ли каталог и просматриваем его содержимое, где видим наш архив. Для просмотра архива введём команду "bunzip2 -c backup.sh.bz2"(рис. @fig:008).

![Проверка действий](image/8.png){#fig:008 width=50%}

2. Создадим файл для второго скрипта и откроем его. (рис. @fig:009).

![Создание файла](image/9.png){#fig:009 width=50%}

Напишем соответсвующий скрипт. (рис. @fig:010).

![Текст скрипта](image/10.png){#fig:010 width=50%}

Проверим работу скрипта. Для этго испольузем команду "./os2.sh "Аргументы"". Но для начала дадим право на выполнение. Введём количество аргументов больше и меньше 10. Скрип работает корректно. (рис. @fig:011).

![Проверка скрипта](image/11.png){#fig:011 width=50%}

3. Создадим файл для написания тертьего скрипта. Также открываем его в "emacs".(рис. @fig:012).

![Создание файла](image/12.png){#fig:012 width=50%}

Напишем командный файл — аналог команды ls (без использования самой этой команды и команды dir).(рис. @fig:013).

![Текст скрипта](image/13.png){#fig:013 width=50%}

Затем даём право на выполнение и првоеряем работу скрипта. (рис. @fig:014).

![Првоерка](image/14.png){#fig:014 width=50%}

4. Для выполнения последнего пунтка создём файл для работу и открываем его. (рис. @fig:015).

![Создание файла](image/15.png){#fig:015 width=50%}

Напишем скрипт по заданию. (рис. @fig:016).

![Текст скрипта](image/16.png){#fig:016 width=50%}

Далее проверим работу. Дадим право на исполнение и в домашнем каталоги создадим пару файлов. Првоерим работу. (рис. @fig:017).
